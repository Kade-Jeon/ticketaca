		...
		<div class="row">
			<div class="col-sm-2">
				...
				<ul class="nav nav-pills flex-column">
					<li class="nav-item"><a class="nav-link text-body" href="${contextPath}/admin/listExhs.do">전체 전시</a></li>
					<li class="nav-item"><a class="nav-link text-body" href="${contextPath}/admin/listExhsOngoing.do">진행 중 전시</a></li>
					<li class="nav-item"><a class="nav-link text-body" href="${contextPath}/admin/listExhsNearingDeadline.do">마감 임박</a></li>
					<li class="nav-item"><a class="nav-link text-body" href="${contextPath}/admin/listExhsUpcoming.do">예정 전시</a></li>
					<li class="nav-item"><a class="nav-link text-body" href="${contextPath}/admin/listExhsClosing.do">종료 전시</a></li>
						...
						<c:when test="${!empty exhsList}">
							...
							<tbody>
								<tr>
									...
									<td class="text-center d-none d-md-table-cell"><a href="${contextPath}/admin/exhDetail.do?no=${exh.no}">${exh.name}</a></td>
									...
									<td class="text-center d-none d-md-table-cell"><a href="${contextPath}/admin/viewExh.do?no=${exh.no}">수정</a></td>
									<td class="text-center d-none d-md-table-cell"><a href="${contextPath}/admin/deleteExh.do?no=${exh.no}">삭제</a></td>
								</tr>
							</tbody>
							</c:forEach>
						</c:when>
								...
								<c:when test="${totalExhs > 100}">	<%-- 전시 개수가 100 초과인 경우 --%>
									...
									<ul class="pagination justify-content-center">
										<c:if test="${section > 1 && page == 1}" >											
											<li class="page-item"><a class="page-link" href="${contextPath}/admin/listExhs.do?section=${section-1}&pageNum=${(section-1) * 10 + 1 }">이전</a></li>
										</c:if>
											<li class="page-item"><a class="page-link" href="${contextPath}/admin/listExhs.do?section=${section}&pageNum=${page}">${(section-1) * 10 + page}></a></li>
										<c:if test="${page == 10}" >
											<li class="page-item"><a class="page-link" href="${contextPath}/admin/listExhs.do?section=${section-1}&pageNum=${(section+1) * 10 + 1 }">다음</a></li>											
										</c:if>
										</ul>
									</c:forEach>
								</c:when>
									...
									<c:forEach var="page" begin="1" end="${totalExhs/10 + 1}" step="1" >
										<c:choose>									
											<c:when test="${page == pageNum}">
												<a class="sel-page" href="${contextPath}/admin/listExhs.do?section=${section}&pageNum=${page}">${page}</a>
											</c:when>
										<c:otherwise>
											<a class="no-uline"  href="${contextPath }/admin/listExhs.do?section=${section}&pageNum=${page}">${page } </a>
										</c:otherwise>
									</c:choose>
								</c:forEach>
							</c:when>
